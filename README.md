...
## Template Driven Form - LEZIONE 29

Per **Template Driven Form** intendiamo quei form che vengono gestiti tutti a livello template, abbiamo la possibilità di lavorarci anche con TypeScript ma di base il form viene costruito e gestito a livello template, ovvero nell'HTML. 

Nel prossimo episodio vedremo i **Reactive Form**, ovvero quei form che vengono costruiti e gestiti lato TypeScript.

Andiamo a "rubare" un bel form da Angular Material.

* A questo link https://material.angular.io/components/form-field/api prendiamo l'API dei Form Field.
* A questo link https://material.angular.io/components/select/api l'API del Select.

Gli import API vanno inseriti in **app.module.ts**.

**home.component.html**

```html
<!-- Simple form fild Angular Material
Nel codice importato manca il tag form, lo dobbiamo aggiungere
noi -->
<form>
...
Non è il classico form HTML che viene inviato automaticamente ad un pagina. Qua il form viene trattato per quello che è, un form. Quando iviamo qualcosa dal form, i dati noi li prendiamo dal TypeScript, che è collegato ad un service. 

In questo form noi mettiamo dentro i dati, premendo su submit inviamo questi dati, questi dati verranno inviati al service che tramite un modulo http manda questi dati al backend, il backend li lavoro e ci restituisce indietro la risposta.    

Questo è diverso dal normale HTML che in genere manda direttamente a una pagina php che risiede nello stesso posto della pagina HTML. 

Ora dobbiamo mettere i **riferimenti ng** (ng vuol dire Angular) per renderlo effettivamente utilizzabile; 

**MINUTO 7:00 DA CONTINUARE**

## CHIAMATE HTTP - EXTRA LIPARI TECH LEZIONE 01
 
INSTALLAZIONE DELLE VARIE DIPENDENZE E CREAZIONE DEL DB FITTIZIO (DA FARE)
 
Avvio del Json Server:
...
**ng g c features/users/components/users-container**
 
**ng g c features/users/components/users-list --skip-test**
 
In core andranno i service module.
 
Potremmo farlo da linea di comando, ma facciamolo a mano, dentro **core** -> **services** -> **user-service.ts**, qua dentro metteremo tutte quante le API che utilizzeremo, andremo a creare un insieme di metodi che utilizzeremo poi nei nostri container per andare a gestire la chiamata HTTP al server. Perché le chiamate le facciamo dentro ai service, mai dentro i componenti. Il component serve solo per andare a gestire il template e i dati, che però ci arrivano da un'altra parte, il service serve a centralizzare funzionalità che potremmo utilizzare in altre parti.
 
Andiamo a radunare tutte queste funzionalità dentro un service, stando però attenti al **concetto di coesione**: non è altro che quel concetto secondo il quale una classe deve essere coerente con quelli che sono i ruoli che noi gli abbiamo assegnato, può avere più metodi ma tutti finalizzati a un determinato scopo, lo user-service non dovrà mai avere metodi che caricano i prodotti ad esempio.
 
Essendo un service va registrato, è un service perché fa parte della gerarchia dei providers. I service degli user li vado a mettere nella cartella core, e va registrato (importato) in **core.module.ts** tra i providers.
...
export class UserService {
    ...
    constructor(
        // se ho bisogno dell'http client devo inniettare:
        private http: HttpClient
    ) {
    }
 
    public loadUsers(): Observable<User[]> {
        // Noi stiamo chiedendo all'http client di darci delle info in
        // modalità asincrona
...
@NgModule({
  ...
  // Se io dichiaro degli oggetti nel core module non sono disponibili al
  // di fuori di esso, devo esportarli per portarli all'esterno del modulo
  exports: [
      ...EXPORTS
  ]
})
...
Se nel service facessi questo injectable:
...
Non avrei bisogno di importare i service in core.module.ts.
 
Il decoratore Injectable è stato creato per far si che il service possa ricevere dipendenze.
 
Richiamiamo il service all'interno del componente users-list:
...
export class UsersListComponent implements OnInit {
  ...
  ngOnInit(): void {
    // subscribe è un metodo degli observable che serve per agganciarti quando
    // arriva la risposta dell'observable che è stato lanciato
    // subscribe in genere prende tre fuzioni: next, error e la complete (questa non viene
    // fatta mai nelle chiamate http)
    ...
    console.log("Dopo loadUsers...");
  }
}
```
 
Tramite il costruttore sono andato a inniettare lo UserService. Che vuol dire **dependency injection**? Vuol dire che quando Angular dovrà creare un'istanza del componente user-list, capirà che il costruttore ha bisogno di una dipendenza, ovvero lo UserService. Quest UserService può essere inniettato se è registrato tramite provider, nel nostro caso in core.module.ts.
 
Da non dimenticare che **core.module.ts** deve essere importato in **app.module.ts**.

Nella cartella  **shared** creiamo la cartella **model** ed inseriamo il seguente file:
...
// Quando dobbiamo creare solo dei modelli di dati usiamo le interfacce
...
Questo User in users-model.ts lo andiamo a inserire come tipo nell'observable in **user-service.ts**. 

Per poter inserire l'users-container in app.component.html abbiamo dovuto prima esportare il container tramite users.module.ts:
...
@NgModule({
  declarations: [UsersContainerComponent, UsersListComponent],
...
Noi esportiamo lo UsersContainerComponent che ha al suo interno del file html il selettore allo UsersListComponent.
